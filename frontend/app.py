...
def load_model_and_results(model_path="model"):
    ...
    with open(f"{model_path}/model_results.json", "r") as f:
        results = json.load(f)
...
def main():
    ...
    try:
        ...
        # Display model performance metrics in a sidebar
        st.sidebar.header("Model Performance")
        st.sidebar.metric("Accuracy", f"{results['accuracy']:.3f}")
        st.sidebar.metric("Precision", f"{results['precision']:.3f}")
        st.sidebar.metric("Recall", f"{results['recall']:.3f}")
        st.sidebar.metric("F1 Score", f"{results['f1_score']:.3f}")
